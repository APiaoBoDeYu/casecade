...
def BKXQS_detector(model_step1, model_step2, imgPath, thres=0.48, min_size=250):
    ...
    # step1_rst = inference_detector(model_step1, img)[0]
    # final_rst = []
    ...
    #     step2_rst = inference_detector(model_step2, tmp_file)
    #     step2_rst = np.array(step2_rst[0])
    #     os.remove(tmp_file)
    ...
    for i in range(len(step1_rst_list)):
        ...
        for k, det in enumerate(step1_rst):
            ...
            if  x1 >= x2 or y1 >= y2 or float(conf)<0.6:#x1 >= x2 or y1 >= y2 :
                continue
