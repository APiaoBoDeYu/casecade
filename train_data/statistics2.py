...
path = "./xml/"#'../xml'#'./train_data/Annotations'
picpath = './photo'
tmppath = '../tmppath/'
...
attrlist=[0,0,0,0]
#import cv2
need_list=["LX","JYZ_SQPS","JYZ_SQZB"]
badlist=['JYZ_C_CZ_type_V','JYZ_C_BL_type_V','JYZ_C_FH_type_V']
...
for roots,dir,files in os.walk(path):
    for file in files:
        ...
        if file.split('.')[-1] == 'xml':
            ...
            respond_pic = os.path.join(roots.replace('xml','photo'),file.split('.')[0]+'.JPG') #这里是不是要改成jpg
